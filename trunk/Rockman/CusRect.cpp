...
CusRect::CusRect( CusRect oldRect, float vx, float vy )
{
	...
	rect.left = oldRect.left+ (LONG)vx;
...
int CusRect::InterSect( CusRect oldRect, float vx, float vy )
{
	...
	float interX, interY;
	if(vx<0){x0 = (float)oldRect.left;X = (float)this->right;}
	else if(vx>0){x0 = (float)oldRect.right;X = (float)this->left;dirX = -1;}

	if(vy<0){y0 = (float)oldRect.top; Y = (float)this->bottom;}
	else if(vy>0){y0 = (float)oldRect.bottom; Y = (float)this->top;dirY = -1;}
	...
	if(vx ==0)
	{
		...
		if(vy <0)
		{
			y1 = (float)oldRect.bottom;
			y0 = y1+vy- oldRect.height;		
		}
		else if(vy>0)
		{
			y0 = (float)oldRect.top;
		...
		if(vy ==0)
		{
			...
			if(vx <0)
			{
				x1 = (float)oldRect.right;
				x0 = x1+vx- oldRect.width;		
			}
			else if(vx>0)
			{
				x0 = (LONG)oldRect.left;
