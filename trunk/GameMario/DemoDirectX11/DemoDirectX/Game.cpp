...
void CGame::_Render()
{
	...
	HRESULT re = _d3ddv->BeginScene();
		_d3ddv->StretchRect(CRescource::GetInstance()->GetSurface(CRescource::bgFileFath),NULL,_backbuffer, NULL,D3DTEXF_NONE);
	if (re) 
	{
		//_d3ddv->StretchRect(CRescource::GetInstance()->GetSurface(IMAGE_BG1), NULL,_backbuffer, NULL,D3DTEXF_NONE);
