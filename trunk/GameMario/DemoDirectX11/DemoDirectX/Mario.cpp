...
CMario::CMario(void)
{
	...
	//	gravity = GRAVITY;
	state = 0;
	sprite = new CSprite(CRescource::GetInstance()->GetSprite(IMAGE_MARIOSMALL), 300, 100, 6, 1);
...
CMario::CMario( D3DXVECTOR3 _pos)
{
	...
	lastvx = 1;
	state = 0;
	spriteCovert = new CSprite(CRescource::GetInstance()->GetSprite(IMAGE_MARIOCONVERT),235,90,5,1);
	spriteCovert->IndexOf(0);
	sprite = new CSprite(CRescource::GetInstance()->GetSprite(IMAGE_MARIOSMALL), 329, 60, 7, 1);
...
void CMario::Render()
{
	...
	else
	{
		...
		if(isSmall) _y-=30;
		//if(isConvert)
		spriteCovert->Render((int)pos.x, (int)_y, lastvx,0.61);
	}
}
bool CMario::IsTurn()
{
...
#pragma endregion 
	...
	if(marioGame->score->Time()<=0) {if(life)this->SetDie();}
...
{
	if(this->pos.y>550&&!isCreep) SetDie();
	checked = true;
	...
	for(CStaticObject* ob:listCollision)
	{
		...
		if(collision!=0)
		{
			if(ob->id ==3||ob->id==4) {this->SetLife(false);break;}
			else
			#pragma region Get items
			//--get item.
			if(ob->id==12){if(((CMushLife*)ob)->state==2){ob->life = false;marioGame->score->SetLife(1);SoundManager::GetInst()->PlayEffSound(SOUND_E_COIN,false);}}
			...
			else if(ob->id==11){ob->life = false;SoundManager::GetInst()->PlayEffSound(SOUND_E_COIN,false);marioGame->score->SetNumCoin();}
			else if(ob->id==-25&&!isWin){((CFlag*)ob)->SetState();pos.x = (float)ob->rectagle.left+16;isWin = true;distance = ob->rectagle.bottom;}
			else if(ob->id==-26){((CHouse*)ob)->SetState(5);this->life = false;}
			//------va ch?m v?i enemy khi là super
			if(isSuper)
			{
				if(ob->id <10&&ob->id>0)
				{((CMotionObject*)ob)->SetLife(false);SoundManager::GetInst()->PlayEffSound(SOUND_E_SLIDE,false);}
			}
#pragma endregion 
			//-----
			if(collision==1)//collision horizontal
			{
				if(ob->rectagle.InterSect(this->rectagle,vx,0)!=0)
				{
					...
					//--
					else if(0<ob->id&&ob->id<10)
					{this->SetLife(false);break;}
			...
			else//collision vertical
			{
				...
				if(this->rectagle.top>=ob->rectagle.bottom)//collision top
				{
					...
					//---va chạm block
					else if(ob->id == -10){ SetPosY(ob);if(isSmall)((CBrick*)ob)->SetPush(true);else ((CBrick*)ob)->SetEx(true) ;vy = 0;break;}//--brick
					...
					else if(ob->id==21||ob->id==22){SetPosY(ob);SoundManager::GetInst()->PlayEffSound(SOUND_E_COIN,false);((CCoin*)ob)->SetState(1);marioGame->score->SetNumCoin();}
					else if(ob->id==23||ob->id==24){SetPosY(ob);SoundManager::GetInst()->PlayEffSound(SOUND_E_COIN,false);;((CCoin1*)ob)->SetNumCoin();marioGame->score->SetNumCoin();}
					...
					if(this->rectagle.bottom<=ob->rectagle.top)//collision bottom
					{
			 			 if(-10<ob->id&&ob->id<-2/*ob->id==-7*/){
							 if(isSitting&&((CPipeVer*)ob)->IsCreep(this,root)) SetCreep(ob,1);
							 else
								 SetPosY(ob);break;}
						//	else
						if(ob->id <0||ob->id>20){ SetPosY(ob);break;}
...
#pragma endregion 
					...
					{
						if(ob->id<0&&ob->id>-3&&!isCreep)
						{SetCreep(ob);}
						else if(-10<ob->id&&ob->id<-2/*ob->id == -7*/&&!isCreep)
						{SetCreep(ob);}
					}
...
void CMario::SetLarge( bool _isSmall )
{
	...
	{
		powerJumb = 55;
		sprite = new CSprite(CRescource::GetInstance()->GetSprite(IMAGE_MARIOLARGE), 329, 90,7, 1);
		SoundManager::GetInst()->PlayEffSound(SOUND_E_GROW,false);
	}
...
void CMario::SetGun()
{
	SoundManager::GetInst()->PlayEffSound(SOUND_E_SLIDE,false);
...
void CMario::SetConvertGun()
{
	...
	isConvert = true;
	isConvertGun = true;
	sprite = new CSprite(CRescource::GetInstance()->GetSprite(IMAGE_MARIOGUN), 282, 90, 6, 1);
}
void CMario::SetCreep(CStaticObject* ob,float _vyCreep)
{
	if(ob->id>-3)//pipe horizontal
	{
		...
		if(ob->id <-2)//pipe vertical
		{
			pos.x = (LONG)ob->rectagle.left+25;
			vxCreep = 0;
			vyCreep = _vyCreep;
			isCreep = true;
			distanceCreep = 0;
...
#pragma region creep horizontal
	...
	state = 1;
	distanceCreep += 4;
	if(distanceCreep>(20+rectagle.width/2)||distanceCreep>distance+rectagle.width/2)
	{	
		timesCreep++;
		if(timesCreep==3)
		{
