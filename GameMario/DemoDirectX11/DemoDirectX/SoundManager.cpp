...
SoundManager::SoundManager()
{
	...
	_listEffSound.push_back(SDev->LoadSound(SOUND_E_PAUSE));
	_listEffSound.push_back(SDev->LoadSound(SOUND_E_BULLETEX));
...
void SoundManager::PlayEffSound(char* SOUND_FILE_PATH, bool isLoop,bool wantPlayAgain)
{
...
bool SoundManager::IsSoundEffPlaying( char *SOUND_FILE_PATH )
{
	int size = _listEffSound.size();
	...
	for(int i = 0 ; i < size; ++i){
		if(strcmp(_listEffSound[i]->m_fileName, SOUND_FILE_PATH) == 0)
		{
			return _listEffSound[i]->IsSoundPlaying();
		}
