...
CPipeVer::CPipeVer( D3DXVECTOR3 _pos,int kind)
{
	...
	sprite = new CSprite(marioGame->spritePipe,200,100,2,2);//horizontal
	SetKind(kind);
}
CPipeVer::~CPipeVer(void)
{
...
void CPipeVer::Render()
{
	...
	for(int i = 0; i<num; i++)
	{
		...
		sprite->Render(pos.x,pos.y+i*50,0, 0.6);
	}
}
...
bool CPipeVer::IsCreep( CMario* mario,CQuadTreeNode *root )
{
	...
	{	
	//	checked = true;
		CusRect nextRect = CusRect(this->pos.x, this->rectagle.bottom+1, mario->rectagle.width, mario->rectagle.height);
